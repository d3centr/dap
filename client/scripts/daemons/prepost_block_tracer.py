...
if __name__ == '__main__':
    ...
    def trace_tx(tx):
        ...
        erc721_txs = sum(anon_event_shield(l) == ERC721_TRANSFER for l in receipt.logs)
        # turn AttributeDict types to plain serializable dict
        receipt = dict(receipt, logs=[dict(log) for log in receipt.logs
            if anon_event_shield(log) == ERC20_TRANSFER])
    ...
    while True:
        ...
        for tx in traces:
            ...
            if receipt_logs:
                ...
                journal.write(pprint.pformat(tx['structLogs']))
                break
